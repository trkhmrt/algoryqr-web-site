...
export async function POST(req: Request, context: { params: Promise<{ menuId: string }> }) {
  try {
    ...
    const upstream = await fetch(`${API_BASE_URL}/menu/${menuId}/products/images`, {
      method: "POST",
      headers,
      body: formData,
    });
...
export async function DELETE(req: Request, context: { params: Promise<{ menuId: string }> }) {
  try {
    const headers = await authHeaders();
    if (!headers) return NextResponse.json({ message: "Access token yok" }, { status: 401 });
    ...
    if (objectKey) params.objectKey = objectKey;
    if (imageUrl) params.imageUrl = imageUrl;

    const upstream = await axios.delete(`${API_BASE_URL}/menu/${menuId}/products/images`, {
      headers,
      params,
      validateStatus: () => true,
      timeout: 20_000,
